...
Like.s<-function(lambda, slugs){
    prod(dpois(slugs, lambda=lambda))
}


#' Log likelihood for single Poisson distribution
minus.logL.s<-function(lambda, slugs){
  -sum(dpois(slugs, lambda=lambda, log=TRUE))  
}
...
for (i in 1:nvals){
  likeslugs[i]<-Like.s(lambda=lambda.test[i], slugs=slugs$slugs)
  loglikeslugs[i]<-minus.logL.s(lambda=lambda.test[i], slugs=slugs$slugs)
}
...
#' Create log-likelihood function
  minus.logL.2<-function(pars, slugsNursery, slugsRookery){
    lambda1<-pars[1]
    lambda2<-pars[2] 
    -(sum(dpois(slugsNursery$slugs, lambda1, log=TRUE)) + 
      sum(dpois(slugsRookery$slugs, lambda2, log=TRUE))) 
  }

#' Now, estimate the lambdas using optim
  mle.fit.2<-optim(par=c(2,2), fn=minus.logL.2, slugsNursery=subset(slugs, field=="Nursery"),
                   slugsRookery = subset(slugs, field=="Rookery"), method="BFGS", hessian=T)  
  mle.fit.2$par  
  varcovmatrix<-solve(mle.fit.2$hessian)
  SEs<-sqrt(diag(varcovmatrix))
  SEs
  
#' Compare to glm
  slugpois<-glm(slugs~field-1, family=poisson(), data=slugs)
  summary(slugpois)
  
hist(subset(slugs, field=="Nursery")$slugs, freq=FALSE)  
