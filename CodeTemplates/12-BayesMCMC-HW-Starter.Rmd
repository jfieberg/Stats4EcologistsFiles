...
```{r fig.height = 6, fig.width = 8, out.width = "65%"}
ggplot(vip.dat, aes(x = length, y = mass, colour = pop)) + 
  geom_point() + geom_smooth(method = "lm")
```

3. The parameters used to simulate the data, `beta.vec` = (-250, 150, 200, 6, -3, -4), correspond to parameters in a model formulated using *effects* coding.  Fit a linear model in R using *effects* coding (the default) and show that you can recover the parameters used to generate the data.
...
All of the 95% confidence intervals include the values of the parameters used to generate the data.
