...
MCMCtrace(out.pois, params="beta",  pdf=FALSE, ind = TRUE)

#' Let's plot Pearson residuals versus fitted values
#' for the Poisson regression model
#+  out.width="55%", fig.alt="Residual plots"
...
#+ fig.alt = "Density and trace plots"
MCMCsummary(out.nb, params = "beta")
MCMCtrace(out.nb, pdf=FALSE, ind = TRUE, params = "beta")
...
## ---------------------------------------------------------------------------------------------
#' ## Alternative prior for theta
#' 
#' Let's try a gamma(1, 0.1) prior. Gamma distributions have support only on positive
#' values.  Also, there is discussion online how weak priors like a gamma(0.01, 0.01)
#' put too much support on values close to 0 (large overdisperions)
x<-rgamma(100000, 0.01, 0.01)
hist(x)
x<-rgamma(100000, 1, 0.01)
hist(x)
...
# Start Gibbs sampler
out.nb2 <- jags.parallel(data = jagsdata, parameters.to.save = params, 
                        model.file = nb.fit2, n.thin = 2, n.chains = 3, n.burnin = 5000, 
                        n.iter = 20000)

#' Density and trace plots
#+ fig.alt = "Density and trace plots"
MCMCsummary(out.nb2, params = "beta")
MCMCtrace(out.nb2, pdf=FALSE, ind = TRUE, params = "beta")
 
#' Compare posteriors for theta using the two different priors
#+ fig.alt="Posterior comparisons of theta"
MCMCplot(out.nb, out.nb2, param = "theta")
...
#' Density and trace plots
#+ fig.alt = "Density and trace plots"
MCMCsummary(out.poisnorm, params = "beta")
MCMCtrace(out.poisnorm, pdf=FALSE, ind = TRUE, params = "beta")
