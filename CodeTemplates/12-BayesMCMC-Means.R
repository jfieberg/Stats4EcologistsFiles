...
#' Typical data set
jawdat<-data.frame(sex = as.factor(c(rep("male", 10), rep("female", 10))),
                   jawlength = c(males, females))
jawdat$pop<-as.numeric(jawdat$sex)
jawdat
...
jaw.mod<-function(){
  ...
  # Priors 
    meanjaw[1] ~ dnorm(100, 0.001) # mean of male jaw lengths
    meanjaw[2] ~ dnorm(100, 0.001) # mean of female jaw lengths
    sigma ~ dunif(0, 30) # common sigma
    tau <- 1/(sigma*sigma) #precision

  # Likelihood (Y | mu) = normal(mu[gender], sigma^2)  
    for(i in 1:n){
      jawlength[i] ~ dnorm(mu[i], tau) 
      mu[i] <- meanjaw[pop[i]]
    } 
  
  # Derived quantities:  difference in means
    mu.diff <- meanjaw[2] - meanjaw[1] 
}
