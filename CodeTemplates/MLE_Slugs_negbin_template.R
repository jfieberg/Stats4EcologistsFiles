...
#' Repeat, but with negative binomial distribution
  minus.logL.s<-function(params, dat){
...
#' it will result in NaN's and thus, the warnings.
  mle.fit<-optim(par=c(2, 10), fn=minus.logL.s,  method="BFGS", hessian=T, dat=slugs$slugs)  
  mle.fit 
