...
# MCMC settings
nc = 3 # NCHAINs
ni = 5000 # number of iterations/chain
nb = 200 # burnin (throw away)
nt = 1 # thinning
...
Prior.beta0<-runif(niters, -10, 10)  

#' Now, call MCMCtrace
MCMCtrace(lmbayes, params = c("beta0"),
          priors = Prior.beta0,
          pdf = FALSE,
          post_zm = FALSE)
...
lmresids <- MCMCpstr(lmbayes, params = "residual", func = mean)$residual
lmfitted <- MCMCpstr(lmbayes, params = "mu", func = mean)$mu
lmsigma <- MCMCpstr(lmbayes, params = "sigma", func = mean)$sigma
jagslmfit <- data.frame(resid = lmresids, 
                        fitted = lmfitted, 
...
#' Pull off sum-of-squared errors (SSE) for observed and simulated data
fitstats <- as.data.frame(MCMCchains(lmbayes, params = c("fit", "fit.new")))
head(fitstats)

#' Determine if SSE for simulated data > SSE for observed data
#'  => model fits the observed data better than simulated data
...
fitstats <- fitstats %>% mutate(postdiff = fit.new-fit)
...
betas <- as.data.frame(MCMCchains(lmbayes, params = c("beta0", "beta1")))
...
for(i in 1:length(prop.black)){
  ...
  mu.hats <- betas$beta0 + rep(prop.black[i], nmcmc)*betas$beta1
  conf.int[i,] <- quantile(mu.hats, prob = c(0.025, 0.975))
}
