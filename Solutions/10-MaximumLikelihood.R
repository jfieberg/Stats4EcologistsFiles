...
#' Likelihood function: need to add the pars, need to add log likelihood
logL<-function(pars, dat){
    ...
    sigi<-sqrt(vari)
    ll<- -sum(dnorm(dat$wtkg, mean = mu, sd = sigi, log = TRUE)) # Log likelihood
    return(ll) 
  }   
...
#'  
fitvb <- optim(par = c(100, 0.23, -0.1, 1.62, 0), 
...
pars<-data.frame(Linf = fitvb$par[1], K = fitvb$par[2], t0 = fitvb$par[3],
                 sigma = exp(fitvb$par[4]), theta = exp(fitvb$par[5]))
