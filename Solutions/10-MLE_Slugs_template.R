...
Like.s2<-function(lambda, slugs){
   # prod(exp(-lambda)*lambda^slugs/factorial(slugs))
 likelihood<-  prod(dpois(slugs, lambda = lambda))
 return(likelihood)
}



#' Log likelihood for single Poisson distribution
logL.s<-function(lambda, slugs){
   loglike <- sum(dpois(slugs, lambda = lambda, log = TRUE))
}
...
likeslugs<-matrix(NA, ncol=nvals, 1) 
loglikeslugs<-matrix(NA, ncol=nvals, 1)
for (i in 1:nvals){
  likeslugs[i] <- Like.s2(lambda.test[i], slugs = slugs$slugs)
  loglikeslugs[i] <- logL.s(lambda.test[i], slugs=slugs$slugs)
}
  
par(mfrow=c(1,2))
  plot(lambda.test, likeslugs, type = "l")
  plot(lambda.test, loglikeslugs, type = "l")
  
#' Instead of using a loop, we can use sapply to calculate the 
#' likelihood for several different values of lambda.
 L.vals<-sapply(1:nvals, FUN=function(x){Like.s2(lambda.test[x], slugs = slugs$slugs)}) 
 logL.vals<-sapply(1:nvals, FUN=function(x){logL.s(lambda.test[x], slugs = slugs$slugs)}) 
...
#'  respectively  
  ...
  lambda.test[which.max(logL.vals)] # Gives us lambda that minimizes -LogL
...
#' Negative Log likelihood for single Poisson distribution. This will let us find
#' a minimum rather than a maximum using R's built in optimizers.
minus.logL.s<-function(lambda, slugs){
    minus.loglike <- -sum(dpois(slugs, lambda = lambda, log = TRUE))
    return(minus.loglike)
}
...
#' it will result in NaN's and thus, the warnings.
  mle.fit<-optim(par=2, fn=minus.logL.s,  method="BFGS", hessian=T, slugs=slugs$slugs)  
  mle.fit 
