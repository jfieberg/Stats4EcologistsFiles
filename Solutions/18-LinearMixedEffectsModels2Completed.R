...
# Centering and scaling helps when fitting the models
RIKZdat$NAPc <-  RIKZdat$NAP - mean(RIKZdat$NAP)

#' Random intercept model
lmer.ri <- lmer(Richness ~ NAP + (1|Beach), data=RIKZdat) #lme4 package
summary(lmer.ri)

lmer.ri <- lmer(Richness ~ NAPc + (1|Beach), data=RIKZdat) #lme4 package
summary(lmer.ri)
...
lmer.rc <- lmer(Richness ~ NAP + (1 + NAP | Beach), data=RIKZdat )
summary(lmer.rc)
...
#' Note, we cannot also include level-2 predictors that are constant
#' within a beach
...
lm.fc2 <- lm(Richness ~ Beach + NAPc + Beach:NAPc - 1 - NAPc, data = RIKZdat) 
summary(lm.fc2)
